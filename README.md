...
We selected [Carbon Cloud API](https://carboncloud.com/climate-footprint-api/) to get products Carbon dioxide equivalent (CO2e). Our application aims to allow users to:
...
My coursemate - Emilie, proposed the idea for the project. We quickly agreed that the project had lots of potential and could be beneficial for users’ general knowledge, rather than just making a practice project showcasing programming skills.
...
We used Slack and Zoom to communicate the updates on the project and what each person was currently working on.
Emilie has focused on styling of the components, creating outlay, working with the data visualisation library. We worked together on data that is being returned from API calls. Below I explain the steps I took to implement functionality of the components I worked on.
